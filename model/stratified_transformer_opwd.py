...
def get_indice_pairs(
    ...
    if i % 2 == 0:
        window_coord = torch.div(
            xyz[new_p2v_map] - xyz_min, window_size, rounding_mode="floor"
        )  # [n, k, 3]
    else:
        window_coord = torch.div(
...
class MemoryCell(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.ConvGates = nn.Linear(
            self.input_size + self.output_size, 2 * self.output_size
        )
        self.Conv_ct = nn.Linear(
    ...
    def forward(
        self, in_feats, xyz, support_xyz, offset, support_offset, support_feats
    ):
        """
        input: n,d
        xyz:
        ...
        new_offset:
        """
        num, _ = in_feats.shape
        hidden = torch.normal(
            1.0,
            0.1,
            (num, self.output_size),
            requires_grad=False,
            device=in_feats.device,
        )
        feats = self.pointnet(in_feats)  # n,d

        c1 = self.ConvGates(torch.cat((feats, hidden), 1))  # n,2d
        (rt, ut) = c1.chunk(2, 1)  # n,
        reset_gate = torch.sigmoid(rt)
        update_gate = torch.sigmoid(ut)
        gated_hidden = torch.mul(reset_gate, hidden)  # n,d
        p1 = self.Conv_ct(torch.cat((feats, gated_hidden), 1))  # n,d
        ct = torch.tanh(p1)
        next_h = torch.mul(update_gate, hidden) + (1 - update_gate) * ct
        next_h = self.linear1(next_h)
        if support_feats is not None:
            next_h = next_h + pointops.interpolation(
                xyz, support_xyz, self.linear2(support_feats), offset, support_offset
            )
        return next_h


class UnknownDet(nn.Module):
    def __init__(self):
        super().__init__()
...
class StratifiedOpwd(nn.Module):
    def __init__(
        self,
        downsample_scale,
        depths,
        ...
        self.memory_cells = nn.ModuleList(
            [
                MemoryCell(channels[i], channels[i - 1] if i != 0 else channels[i])
    ...
    def forward(self, feats, xyz, offset, batch, neighbor_idx):
        ...
        for i, upsample in enumerate(self.upsamples):
            anon_xyz, anon_offset = xyz, offset
            support_xyz, support_offset, support_feats = (
                xyz_stack.pop(),
                offset_stack.pop(),
                feats_stack.pop(),
            )
            ...
            anonymous = self.memory_cells[i](
                feats,
                anon_xyz,
                support_xyz,
                anon_offset,
                support_offset,
                anonymous_feats[-1] if anonymous_feats else None,
            )
            anonymous_feats.append(anonymous)

        out = self.classifier(feats)
        confidence = self.unknown_det(anonymous_feats[-1])

        return out, {"unknown":confidence, "unknown_dist":anonymous_feats[-1]}
