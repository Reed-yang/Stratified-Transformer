...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/scannetv2/scannetv2_stratified_transformer_debug.yaml',
                        help='config file')
    parser.add_argument('opts', help='see config/scannetv2/scannetv2_stratified_transformer_debug.yaml for all options', default=None,
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    elif args.arch == 'stratified_transformer_real':

        from model.stratified_transformer_real import Stratified
    ...
    if args.data_name == 's3dis':
        ...
        train_data = S3DIS(split='train', data_root=args.data_root, test_area=args.test_area,
                           voxel_size=args.voxel_size, voxel_max=args.voxel_max, transform=train_transform,
                           shuffle_index=True, loop=args.loop)
    ...
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, shuffle=(train_sampler is None),
                                               ...
                                               collate_fn=partial(collate_fn_limit,
    ...
    if args.scheduler == "MultiStepWithWarmup":
        ...
        milestones = [int(x) for x in args.milestones.split(",")] if hasattr(args, "milestones") else [
            int(args.epochs * 0.6), int(args.epochs * 0.8)]
        gamma = args.gamma if hasattr(args, 'gamma') else 0.1
        if main_process():
            logger.info(
                "scheduler: MultiStep. scheduler_update: {}. milestones: {}, gamma: {}".format(args.scheduler_update,
                                                                                               milestones, gamma))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_train, mIoU_train, mAcc_train, allAcc_train, aupr, auroc = train(train_loader, model, criterion, optimizer, epoch,
                                                                 scaler, scheduler)
        if args.scheduler_update == 'epoch':
            scheduler.step()
        epoch_log = epoch + 1
        ...
        if main_process():
            ...
            writer.add_scalar("aupr_batch_mean_train", aupr, epoch_log)
            writer.add_scalar("auroc_batch_mean_train", auroc, epoch_log)
        is_best = False
...
def train(train_loader, model, criterion, optimizer, epoch, scaler, scheduler):
    ...
    # ):  # (n, 3), (n, c), (n), (b)
        ...
        output_softmax = F.softmax(output_softmax, dim=-1)
...
def validate(val_loader, model, criterion):
    ...
    if main_process():
        ...
        for i in range(args.classes):
            ...
            logger.info(f"Val AUPR/AUROC batch mean: {aupr_meter.avg:.4f}/{auroc_meter.avg:.4f}")
