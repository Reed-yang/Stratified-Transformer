...
class Scannetv2(Dataset):
    def __init__(self, split='train', data_root='trainval', voxel_size=0.04, voxel_max=None, transform=None,
                 shuffle_index=False, loop=1):
        super().__init__()
        ...
        self.loop = loop
        if isinstance(self.loop, float):
            print("\033[1;31mPlease make sure run in debug mode now!\033[0m", self.loop)
    ...
    def __len__(self):
        ...
        data_len = len(self.data_list) * self.loop
        return data_len if self.loop >= 1 else int(data_len)
